...
class RegRBM(RBM):
    ...
    def _initialize_vars(self):
        ...
        b_reg_grad = tf.reduce_mean(reg_grad, 0) / tf.dtypes.cast((2 * tf.square(self.sigma)), tf.float32)

        def f(x_old, x_new):
            return self.momentum * x_old + \
                   self.learning_rate * x_new * (1 - self.momentum) / tf.cast(tf.shape(x_new)[0],tf.float32)
